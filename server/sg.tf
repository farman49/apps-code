...
resource "aws_security_group" "sh_sg_for_elb" {
  name   = "${var.environment}-sg_for_elb"
...
resource "aws_security_group" "sh_sg_for_ec2" {
  name   = "${var.environment}-sg_for_ec2"
  vpc_id = aws_vpc.sh_main.id
  ...
  ingress {
    description     = "Allow http request from Load Balancer"
    protocol        = "tcp"
    from_port       = 8000 # range of
    to_port         = 8000 # port numbers
    security_groups = [aws_security_group.sh_sg_for_elb.id]
  }
  ingress {
    description     = "Allow http request from Load Balancer"
    protocol        = "tcp"
    from_port       = 8002 # range of
    to_port         = 8002 # port numbers
    security_groups = [aws_security_group.sh_sg_for_elb.id]
  }
